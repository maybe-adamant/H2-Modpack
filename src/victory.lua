...
local MetaUpgradeDisplay = {
    -- Area: between trait tray (left) and stats panel (right), below pin panel
    StartY = 895,
    SpacingX = 40,
    SpacingY = 50,
    Columns = 25,
    IconScale = 0.135,
    IconCenterOffsetX = 20, -- manual X offset to visually center icons (half rendered width)
    Group = "Combat_Menu_TraitTray_Overlay",
    Components = {},
    -- Hover scales (reference: trait tray meta icon at 0.21)
    RefIconScale = 0.21,
    BaseHighlightAnimScale = 0.33,
    BaseTrayHighlightAnimScale = 1.5,
    BasePinIconScale = 0.12,       -- icon size in hover tooltip
    BasePinIconFrameScale = 0.4,   -- frame size in hover tooltip
    HighlightAnim = "DevCard_Hover",
}

local ShrineUpgradeDisplay = {
    GapAfterMeta = 10, -- extra vertical gap between meta and shrine sections
    SpacingX = 50,
    SpacingY = 50,
    Columns = 17,
    IconScale = 0.2,
    ...
    Group = "Combat_Menu_TraitTray_Overlay",
    Components = {},
    -- Hover scales (reference: trait tray shrine icon at 0.4)
    RefIconScale = 0.4,
    BaseHighlightAnimScale = 0.27,
    BaseTrayHighlightAnimScale = 1.4,
    BasePinIconScale = 0.28,       -- icon size in hover tooltip
    BasePinIconFrameScale = 0.0,   -- no frame for shrine icons
    HighlightAnim = "GUI\\Screens\\Shrine\\PactHover",
    -- Rank pips
    PipOffsetY = 22,       -- distance below icon center
    PipSpacing = 10,       -- horizontal spacing between pips
    PipFilled = "●",
    PipEmpty = "○",
    PipFontSize = 12,
    PipFilledColor = { 255, 210, 80, 255 },  -- gold
    PipEmptyColor = { 140, 140, 140, 180 },  -- dim grey
}
...
local function CreateMetaUpgradeDisplay()
    ...
    -- Collect all equipped cards first
    local equippedCards = {}
...
local function CreateShrineUpgradeDisplay( startY )
    ...
    -- Collect active shrine upgrades
    local activeShrines = {}
    ...
    while shrineIndex <= totalShrines do
        ...
        for i = 0, rowCount - 1 do
            ...
            -- Rank pips: one component per pip for individual coloring
...
modutil.mod.Path.Wrap( "OpenRunClearScreen", function( base )
    if config.ModEnabled and config.QoLSettings.ShowArcanaAndFearVictoryScreen then
        thread( function()
            wait( 0.5 ) -- wait for the screen to finish building
...
-- Preserve our icons when the trait tray switches tabs
modutil.mod.Path.Wrap( "TraitTrayScreenRemoveItems", function( base, screen )
    -- Pull our icons out of screen.Icons before the base wipes them
    local savedIcons = {}
    for _, id in ipairs( MetaUpgradeDisplay.Components ) do
    ...
    -- Re-add our icons after the wipe
